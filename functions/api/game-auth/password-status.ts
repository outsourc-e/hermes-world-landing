// Cloudflare Pages Function: POST /api/game-auth/password-status
// HW_PWSTATUS_v1: reports {exists, custom, pending} for the JWT identity's game account.
// Used by the auth callback to force Google users to choose a game password.
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    const r = await fetch(`${bridgeBase}/password-status`, {
      method: 'POST',
